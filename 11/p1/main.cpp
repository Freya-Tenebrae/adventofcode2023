...
class Universe
{
	...
	public:
		...
		std::vector<int> getAllDistancesBeetweenGalaxy(bool debug) const;
...
void Universe::expand()
{
	...
	this->_sizeX += row_to_expand.size();
	this->_sizeY += line_to_expand.size();
...
std::vector<int> Universe::getAllDistancesBeetweenGalaxy(bool debug) const
{
...
void resolve(Universe &universe, int &total, int debug)
{
	universe.expand();
	...
	std::vector<int> distances = universe.getAllDistancesBeetweenGalaxy(debug);
...
int main(int argc, char const *argv[])
{
	...
	std::cout << "the sum of the distance beetween each pair of galaxy is when expension is 2 is : \033[1;33m";
	std::cout << total;
	std::cout << "\033[0m" << std::endl;
	return (0);
}