...
void resolve(std::vector<std::string> inputs, int &total, bool debug, int lineLen, int nLines)
{
	for (int i = 0; i < nLines; i++)
	{
		...
		{
			if (inputs[i][j] != '.' && (inputs[i][j] < '0' || inputs[i][j] > '9'))
			{
				int totalMotor = 0;
					...
					for (int y = j - 1; y <= j + 1; y++)
					{
							...
							isAlredyDoneForThisMotor(NumberDoneForThisMotor, x, y) == false)
						{
							...
							if (debug == true)
							{
								...
								std::cout << "\033[0m][\033[0;34m";
								std::cout << y;
								std::cout << "\033[0m], symbol \033[0;34m'";
								std::cout << inputs[i][j];
		...
		if (debug == true)
		{
			...
			std::cout << inputs[i];
			std::cout << "\033[0m, totalLine = \033[0;34m";
			std::cout << totalLine;
			std::cout << "\033[0m," << std::endl;
		}
	}
...
int main(int argc, char const *argv[])
{
	...
	std::cout << "The sum of all of the part numbers in the engine schematic is : \033[1;33m";
	std::cout << total;
	std::cout << "\033[0m" << std::endl;
	return (0);
}
