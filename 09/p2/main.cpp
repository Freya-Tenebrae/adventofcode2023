...
void	Report::computeExtrapolatedValue()
{
	...
	{
		newValue = this->_logicalSequences[i].front() - newValue;

		std::vector<long> newActualLogicalSequence;
		newActualLogicalSequence.push_back(newValue);
		for (std::vector<long>::iterator it = this->_logicalSequences[i].begin(); it != this->_logicalSequences[i].end(); it++)
			newActualLogicalSequence.push_back(newValue);
		this->_logicalSequences[i] = newActualLogicalSequence;
	}

	newValue = this->_logicalSequences[0].front() - newValue;

	std::vector<long> newActualLogicalSequence;

	newActualLogicalSequence.push_back(newValue);
	for (std::vector<long>::iterator it = this->_logicalSequences[0].begin(); it != this->_logicalSequences[0].end(); it++)
		newActualLogicalSequence.push_back(newValue);
	this->_logicalSequences[0] = newActualLogicalSequence;
...
void resolve(std::vector<Report> reports, long &total, bool debug)
{
	for (size_t i = 0; i < reports.size(); i++)
	{
		...
		if (debug == true)
		{
			...
			std::cout << "\033[0m for the previous reading !";
			std::cout << std::endl;
		}
	}
}
